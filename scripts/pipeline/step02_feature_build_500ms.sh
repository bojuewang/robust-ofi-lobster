...
# Step 02: Build 500ms LOBSTER feature tensors for robust OFI project.
# Contract:
#   - Requires Step 01 PASS gate.
#   - Reads five LOBSTER level-10 sample zip files from data/raw.
#   - Constructs raw X_t in R^{5 x 51}, standardized X_t, masks, mid/spread,
#     and targets for tau in {1,2,10,20}.
#   - Writes benchmark report and Step 02 PASS gate only if all checks pass.

ROOT_CANDIDATE="${PROJECT_DIR:-$(pwd)}"
if [[ -d "${ROOT_CANDIDATE}/data/raw" ]]; then
  ROOT="${ROOT_CANDIDATE}"
elif [[ -d "$(pwd)/robust-ofi-lobster/data/raw" ]]; then
  ROOT="$(pwd)/robust-ofi-lobster"
elif [[ -d "$(pwd)/data/raw" ]]; then
  ROOT="$(pwd)"
else
  echo "[ERROR] Cannot locate project root. Expected data/raw under PROJECT_DIR, ./robust-ofi-lobster, or current directory." >&2
  exit 2
fi

cd "$ROOT"
mkdir -p scripts src/lobster_ofi data/processed reports/benchmarks .pipeline_state

if [[ ! -f ".pipeline_state/step01_raw_audit.PASS" ]]; then
  echo "[ERROR] Step 01 gate missing: .pipeline_state/step01_raw_audit.PASS" >&2
  echo "        Run and pass step01_lobster_raw_audit_clean.sh first." >&2
  exit 3
fi

python - <<'PY_CHECK' >/dev/null 2>&1 || {
import numpy, pandas
PY_CHECK
  echo "[INFO] Installing required Python packages: numpy pandas" >&2
  python -m pip install --upgrade pip >/dev/null
  python -m pip install numpy pandas >/dev/null
}

cat > scripts/step02_feature_build_500ms.py <<'PY'
...
python scripts/step02_feature_build_500ms.py

echo
cat reports/benchmarks/step02_feature_build_500ms.md
