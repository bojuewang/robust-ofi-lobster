...
# Step 05: Graph-robust PGDA training and calibrated graph perturbation evaluation
#
# This module tests the rho_G > 0 component of the proposal:
#   min_theta max_{Delta in U_G(G)} loss(f_theta(X, G + Delta), Y).
#
# It uses a graph-linear model with a fixed base graph from Step 03 and
# calibrated normalized PGD directions for the inner graph perturbation.
#
# Inputs:
#   .pipeline_state/step04b_calibrated_eval.PASS
#   data/processed/features_500ms_step02.npz
#   data/processed/graphs_step03.npz
#
# Outputs:
#   data/processed/graph_robust_results_step05.csv
#   reports/benchmarks/step05_graph_robust_pgda.md
...
# or:
#   PROJECT_DIR=/path/to/robust-ofi-lobster bash step05_graph_robust_pgda.sh

ROOT_DIR="${ROOT_DIR:-$(pwd)}"
PROJECT_DIR="${PROJECT_DIR:-${ROOT_DIR}/robust-ofi-lobster}"

if [[ ! -d "${PROJECT_DIR}" ]]; then
  if [[ -f "${ROOT_DIR}/data/processed/features_500ms_step02.npz" ]]; then
...
if [[ ! -f ".pipeline_state/step04b_calibrated_eval.PASS" ]]; then
  echo "[ERROR] Step 04b gate missing: .pipeline_state/step04b_calibrated_eval.PASS"
  echo "        Run Step 04b first and verify calibrated degradation is nonzero."
  exit 1
fi

if [[ ! -s "data/processed/features_500ms_step02.npz" ]]; then
  echo "[ERROR] Missing data/processed/features_500ms_step02.npz"
  exit 1
fi

if [[ ! -s "data/processed/graphs_step03.npz" ]]; then
  echo "[ERROR] Missing data/processed/graphs_step03.npz"
  exit 1
fi

mkdir -p src/lobster_ofi reports/benchmarks data/processed .pipeline_state

cat > src/lobster_ofi/step05_graph_robust_pgda.py <<'PY'
...
python src/lobster_ofi/step05_graph_robust_pgda.py

echo
echo "[DONE] Step 05 report:"
echo "  ${PROJECT_DIR}/reports/benchmarks/step05_graph_robust_pgda.md"
