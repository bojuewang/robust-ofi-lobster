...
# Step 01 gate: raw LOBSTER sample audit + message cleaning + 500ms event-mask benchmark.
# This is intentionally the FIRST module only. Do not run feature/model modules until this gate passes.
#
# Usage:
#   bash step01_lobster_raw_audit_clean.sh
# Optional:
#   RAW_DIR=/path/to/lobster_zips PROJECT_DIR=robust-ofi-lobster bash step01_lobster_raw_audit_clean.sh
#   AUTO_INSTALL=0 bash step01_lobster_raw_audit_clean.sh

PROJECT_DIR="${PROJECT_DIR:-robust-ofi-lobster}"
RAW_DIR_INPUT="${RAW_DIR:-}"
AUTO_INSTALL="${AUTO_INSTALL:-1}"
PYTHON_BIN="${PYTHON_BIN:-python3}"

mkdir -p "$PROJECT_DIR"
cd "$PROJECT_DIR"
mkdir -p data/raw data/interim data/processed reports/benchmarks logs scripts src/lobster_ofi tests .pipeline_state

# Bring zips into data/raw if possible.
if [[ -n "$RAW_DIR_INPUT" ]]; then
  cp -n "$RAW_DIR_INPUT"/LOBSTER_SampleFile_*_2012-06-21_10*.zip data/raw/ 2>/dev/null || true
else
  cp -n ../LOBSTER_SampleFile_*_2012-06-21_10*.zip data/raw/ 2>/dev/null || true
  cp -n ./LOBSTER_SampleFile_*_2012-06-21_10*.zip data/raw/ 2>/dev/null || true
fi
...
cat > README.md <<'MD'
# Robust OFI LOBSTER Pipeline

This repository follows a gated, reproducible workflow.

## Step 01: raw audit + cleaning gate

```bash
bash scripts/01_raw_audit_clean.sh
```

Inputs: five LOBSTER level-10 sample zip files under `data/raw/`.

Outputs:

```text
data/interim/event_summary_<TICKER>.csv
reports/benchmarks/step01_raw_audit_clean.json
reports/benchmarks/step01_raw_audit_clean.md
.pipeline_state/step01_raw_audit.PASS
```

Paste `reports/benchmarks/step01_raw_audit_clean.md` back into ChatGPT before running Step 02.
MD

cat > src/lobster_ofi/__init__.py <<'PY'
PY

cat > src/lobster_ofi/step01_raw_audit_clean.py <<'PY'
...
cat > scripts/01_raw_audit_clean.sh <<'BASH1'
#!/usr/bin/env bash
set -euo pipefail
PYTHON_BIN="${PYTHON_BIN:-python3}"
...
BASH1
chmod +x scripts/01_raw_audit_clean.sh

cat > scripts/99_prepare_github_after_step01.sh <<'BASH99'
#!/usr/bin/env bash
set -euo pipefail
...
BASH99
chmod +x scripts/99_prepare_github_after_step01.sh
