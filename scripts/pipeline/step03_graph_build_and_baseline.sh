...
# Step 03: Cross-asset graph construction + ERM baselines
# This script assumes Step 02 has produced:
#   data/processed/features_500ms_step02.npz
# and .pipeline_state/step02_feature_build.PASS
#
# It builds:
#   1. fixed training-period return correlation graph
#   2. fixed training-period OFI correlation graph
#   3. fixed lagged cross-OFI graph
#   4. mask-aware versions
#   5. Ridge/Lasso own-feature baselines
#   6. graph-linear ERM baselines
#
# Run:
#   bash step03_graph_build_and_baseline.sh
# or:
#   PROJECT_DIR=/path/to/robust-ofi-lobster bash step03_graph_build_and_baseline.sh
...
if [[ ! -f ".pipeline_state/step02_feature_build.PASS" ]]; then
  echo "[ERROR] Step 02 gate missing: .pipeline_state/step02_feature_build.PASS"
  echo "        Run Step 02 first and verify its benchmark PASS."
  exit 1
fi

FEATURE_NPZ="data/processed/features_500ms_step02.npz"
if [[ ! -s "${FEATURE_NPZ}" ]]; then
  echo "[ERROR] Missing Step 02 feature file: ${FEATURE_NPZ}"
  exit 1
fi

mkdir -p src/lobster_ofi scripts reports/benchmarks data/processed .pipeline_state

cat > src/lobster_ofi/step03_graph_build_and_baseline.py <<'PY'
...
python src/lobster_ofi/step03_graph_build_and_baseline.py

echo
echo "[DONE] Step 03 report:"
echo "  ${PROJECT_DIR}/reports/benchmarks/step03_graph_build_and_baseline.md"
