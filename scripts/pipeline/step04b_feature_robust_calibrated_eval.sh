...
# Step 04b: Calibrated adversarial evaluation for feature robustness
#
# Why this module exists:
#   Step 04 may pass engineering checks while reporting zero degradation because
#   raw gradient-ascent perturbations are too small. This module recalibrates the
#   adversarial test perturbation by using normalized steepest-ascent directions
#   on the standardized feature tensor.
#
# Inputs:
...
#
# Outputs:
#   data/processed/feature_robust_calibrated_eval_step04b.csv
#   reports/benchmarks/step04b_feature_robust_calibrated_eval.md
...
if [[ ! -f ".pipeline_state/step04_feature_robust_pgda.PASS" ]]; then
  echo "[ERROR] Step 04 gate missing: .pipeline_state/step04_feature_robust_pgda.PASS"
  echo "        Run Step 04 first."
  exit 1
fi

mkdir -p src/lobster_ofi reports/benchmarks data/processed .pipeline_state

cat > src/lobster_ofi/step04b_feature_robust_calibrated_eval.py <<'PY'
...
python src/lobster_ofi/step04b_feature_robust_calibrated_eval.py

echo
echo "[DONE] Step 04b report:"
echo "  ${PROJECT_DIR}/reports/benchmarks/step04b_feature_robust_calibrated_eval.md"
